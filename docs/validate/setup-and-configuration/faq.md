...
Check the FAQ for running a validator on Openverse.
...
Openverse is powered by [Tendermint](https://docs.tendermint.com/v0.34/introduction/what-is-tendermint.html) Core,
...
Openverse is a public Proof-of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of
...
and *precommits*.
- **Validator's Address**: `openversevaloper1-` address. This is the address used to identify your validator publicly.
...
--pubkey openversevalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
--amount "2aopenverse"
--from tmp
...
`openversevalconspub`.
    - Generated when the node is created with `versed init`.
    - Get this value with `versed tendermint show-validator`
e.g. `openversevalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`
...
transactions. Application keys are associated with a public key `openversepub-` and an address `openverse-`. Both
are derived from account keys generated by `versed keys add`.

:::warning
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this
purpose: `openversevaloper` and `openversevaloperpub`
:::
...
If you want to obtain coins for the testnet, you can do so by using the [faucet](https://faucet.openverse.dev/).
...
a good practice for validators to have their setup audited by third parties. Note though, that the Openverse team will
...
- **Provide oversight and feedback on correct deployment of community pool funds:** the Openverse protocol includes the a
...
- **Block rewards:** Native tokens of applications run by validators (e.g. EVMOS on Openverse) are inflated to produce
...
- **Transaction fees:** Openverse maintains a whitelist of token that are accepted as fee payment. The initial fee token is
the `openverse`.
...
Openverse enforces the parameter that each validator sets. These parameters can only be defined when initially declaring
...
- [Learnings from BlockDaemon](https://blockdaemon.com/documentation/openverse-post-mortem/)
...
There is an awesome guide. Polkachu is a validator on Openverse and they have wrote [this page](https://github.com/polkachu/validator-guide/blob/main/validator_server_migration_best_practice.md)
to help out.
...
much mining power the community usually stops contributing to that pool. Openverse will rely on the same effect initially.
...
In addition to running an Openverse node, validators should develop monitoring, alerting and management solutions.
...
Openverse has the capacity for very high throughput compared to chains like Ethereum or Bitcoin.
...
The Openverse team does not recommend one solution above the other. The community is encouraged to bolster the effort to
