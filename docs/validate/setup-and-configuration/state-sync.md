...
```bash
SNAP_RPC1="http://bd-openverse-testnet-state-sync-node-01.bdnodes.net:26657"
SNAP_RPC="http://bd-openverse-testnet-state-sync-node-02.bdnodes.net:26657"
CHAIN_ID="openverse_9000-4"
PEER="3a6b22e1569d9f85e9e97d1d204a1c457d860926@bd-openverse-testnet-seed-node-01.bdnodes.net:26656"
wget -O $HOME/genesis.json https://archive.openverse.dev/openverse_9000-4/genesis.json 
```

## Use commands below for Mainnet setup

```bash
SNAP_RPC1="http://bd-openverse-mainnet-state-sync-us-01.bdnodes.net:26657"
SNAP_RPC="http://bd-openverse-mainnet-state-sync-eu-01.bdnodes.net:26657"
CHAIN_ID="openverse_9001-2"
PEER="96557e26aabf3b23e8ff5282d03196892a7776fc@bd-openverse-mainnet-state-sync-us-01.bdnodes.net,dec587d55ff38827ebc6312cedda6085c59683b6@bd-openverse-mainnet-state-sync-eu-01.bdnodes.net"
wget -O $HOME/genesis.json https://archive.openverse.network/mainnet/genesis.json 
```
...
```bash
versed init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .versed/config folder

```bash
mv $HOME/genesis.json ~/.versed/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.versed/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.versed/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.versed/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.versed/config/config.toml
```

### Create versed service

```bash
echo "[Unit]
Description=Versed Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which versed) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/versed.service; sudo mv $HOME/versed.service /etc/systemd/system/
```

```bash
sudo systemctl enable versed.service && sudo systemctl daemon-reload
```

### Run versed

```bash
sytemctl start versed
```

### Check logs

```bash
journalctl -u versed -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.versed/config/config.toml
```
