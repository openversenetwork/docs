...
Before beginning, ensure you have an Openverse node running in the background of the same machine that you intend to relay on.
 Follow [this guide](./../protocol/openverse-cli/single-node) to set up an Openverse node if you have not already.

In this guide, we will be relaying between [Openverse (channel-3) and Cosmos Hub (channel-292)](https://www.mintscan.io/openverse/relayers).
 When setting up your Openverse and Cosmos full nodes, be sure to offset the ports being used in both the `app.toml` and `config.toml`
  files of the respective chains (this process will be shown below).

In this example, the default ports for Openverse will be used, and the ports of the Cosmos Hub node will be manually changed.

## Openverse Daemon Settings

First, set `grpc server` on port `9090` in the `app.toml` file from the `$HOME/.versed/config` directory:

```bash
vim $HOME/.versed/config/app.toml
```
...
Then, set the `pprof_laddr` to port `6060`, `rpc laddr` to port `26657`, and `prp laddr` to `26656` in the `config.toml`
 file from the `$HOME/.versed/config` directory:

```bash
vim $HOME/.versed/config/config.toml
```
...
```bash
# In this example, we will set channel-292 on the cosmoshub-4 chain settings and channel-3 on the openverse_9001-2 chain settings:
[[chains]]
id = 'cosmoshub-4'
...
policy = 'allow'
list = [
   ['transfer', 'channel-292'], # openverse_9001-2
]
...
hermes keys restore openverse_9001-2 -m "24-word mnemonic seed"
```
...
INFO ThreadId(01) [openverse_9001-2] chain is healthy
```
...
```bash
hermes query packet unreceived-packets openverse_9001-2 transfer channel-3
hermes query packet unreceived-acks openverse_9001-2 transfer channel-3
```
...
hermes query packet commitments openverse_9001-2 transfer channel-3
```
...
hermes clear packets openverse_9001-2 transfer channel-3
```
...
```bash
hermes tx raw packet-recv openverse_9001-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-ack openverse_9001-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-recv cosmoshub-4 openverse_9001-2 transfer channel-3
hermes tx raw packet-ack cosmoshub-4 openverse_9001-2 transfer channel-3
```
