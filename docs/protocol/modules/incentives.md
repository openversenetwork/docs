...
This document specifies the internal `x/incentives` module of the Openverse Hub.

The `x/incentives` module is part of the Openverse tokenomics and aims
to increase the growth of the network by distributing rewards
to users who interact with incentivized smart contracts.
The rewards drive users to interact with applications on Openverse and reinvest their rewards in more services in the network.
...
Find below a list of `versed` commands added with the `x/incentives` module.
...
```go
versed query incentives incentives [flags]
```
...
```go
versed query incentives incentive CONTRACT_ADDRESS [flags]
```
...
```bash
versed query incentives gas-meters CONTRACT_ADDRESS [flags]
```
...
```go
versed query incentives gas-meter CONTRACT_ADDRESS PARTICIPANT_ADDRESS [flags]
```
...
```bash
versed query incentives params [flags]
```
...
```bash
versed tx gov submit-legacy-proposal register-incentive CONTRACT_ADDRESS ALLOCATION EPOCHS [flags]
```
...
```bash
versed tx gov submit-legacy-proposal cancel-incentive CONTRACT_ADDRESS [flags]
```
...
| ------ | ---------------------------------------------------------- | --------------------------------------------- |
| `gRPC` | `openverse.incentives.v1.Query/Incentives`                     | Gets all registered incentives                |
| `gRPC` | `openverse.incentives.v1.Query/Incentive`                      | Gets incentive for a given contract           |
| `gRPC` | `openverse.incentives.v1.Query/GasMeters`                      | Gets gas meters for a given incentive         |
| `gRPC` | `openverse.incentives.v1.Query/GasMeter`                       | Gets gas meter for a given incentive and user |
| `gRPC` | `openverse.incentives.v1.Query/AllocationMeters`               | Gets all allocation meters                    |
| `gRPC` | `openverse.incentives.v1.Query/AllocationMeter`                | Gets allocation meter for a denom             |
| `gRPC` | `openverse.incentives.v1.Query/Params`                         | Gets incentives params                        |
| `GET`  | `/openverse/incentives/v1/incentives`                          | Gets all registered incentives                |
| `GET`  | `/openverse/incentives/v1/incentives/{contract}`               | Gets incentive for a given contract           |
| `GET`  | `/openverse/incentives/v1/gas_meters`                          | Gets gas meters for a given incentive         |
| `GET`  | `/openverse/incentives/v1/gas_meters/{contract}/{participant}` | Gets gas meter for a given incentive and user |
| `GET`  | `/openverse/incentives/v1/allocation_meters`                   | Gets all allocation meters                    |
| `GET`  | `/openverse/incentives/v1/allocation_meters/{denom}`           | Gets allocation meter for a denom             |
| `GET`  | `/openverse/incentives/v1/params`                              | Gets incentives params                        |
