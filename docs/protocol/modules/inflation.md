...
The `x/inflation` module mints new Openverse tokens and allocates them in daily
epochs according to the [Openverse Token
Model](https://openverse.blog/the-openverse-token-model-edc07014978b) distribution to
...
The allocation of new coins incentivizes specific behaviour in the Openverse
...
### Openverse Token Model

The Openverse Token Model outlines how the Openverse network is secured through a
...
issue 1 billion Openverse tokens within the first 4 years.
...
daily provision (`epochProvison`) of Openverse tokens is minted and allocated to staking rewards,
...
| ------------------------              | ---------------------- | ----------------------------------------------------------------------------- |
| `ParamStoreKeyMintDenom`              | string                 | `evm.DefaultEVMDenom` // “aopenverse”                                             |
...
the distribution from the Openverse Token Model is recalculated into a distribution
that excludes team vesting. Note, that this does not change the inflation
proposed in the Openverse Token Model. Each `InflationDistribution` can be
calculated like this:

```markdown
stakingRewards = openverseTokenModelDistribution / (1 - teamVestingDistribution)
0.5333333      = 40%                         / (1 - 25%)
```
...
Find below a list of `versed` commands added with the `x/inflation` module. You
can obtain the full list by using the `versed -h` command.
...
```bash
versed query inflation period [flags]
```
...
```bash
versed query inflation epoch-mint-provision [flags]
```
...
```bash
versed query inflation skipped-epochs [flags]
```
...
```bash
versed query inflation total-supply [flags]
```
...
```bash
versed query inflation inflation-rate [flags]
```
...
```bash
versed query inflation params [flags]
```
...
```bash
versed tx gov submit-legacy-proposal param-change [proposal-file] [flags]
```
...
| ------ | --------------------------------------------- | --------------------------------------------- |
| `gRPC` | `openverse.inflation.v1.Query/Period`             | Gets current inflation period                 |
| `gRPC` | `openverse.inflation.v1.Query/EpochMintProvision` | Gets current inflation epoch provisions value |
| `gRPC` | `openverse.inflation.v1.Query/Params`             | Gets current inflation parameters             |
| `gRPC` | `openverse.inflation.v1.Query/SkippedEpochs`      | Gets current number of skipped epochs         |
| `gRPC` | `openverse.inflation.v1.Query/TotalSupply`        | Gets current total supply                     |
| `gRPC` | `openverse.inflation.v1.Query/InflationRate`      | Gets current inflation rate                   |
| `GET`  | `/openverse/inflation/v1/period`                  | Gets current inflation period                 |
| `GET`  | `/openverse/inflation/v1/epoch_mint_provision`    | Gets current inflation epoch provisions value |
| `GET`  | `/openverse/inflation/v1/skipped_epochs`          | Gets current number of skipped epochs         |
| `GET`  | `/openverse/inflation/v1/total_supply`          | Gets current total supply                     |
| `GET`  | `/openverse/inflation/v1/inflation_rate`          | Gets current inflation rate                   |
| `GET`  | `/openverse/inflation/v1/params`                  | Gets current inflation parameters             |
