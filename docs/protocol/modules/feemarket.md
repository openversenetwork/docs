...
You might encounter both terminologies on Openverse since it builds Ethereum on top of the SDK,
...
Thus, the tip for an EIP-1559 transaction on Openverse should be zero
(`MaxPriorityFeePerGas` JSON-RPC endpoint returns `0`).
Have a look at the [mempool](https://docs.openverse.network/validate/setup-and-configuration/mempool) docs
...
```bash
versed query feemarket base-fee [flags]
```

Example:

```bash
versed query feemarket base-fee ...
```
...
```bash
versed query feemarket block-gas [flags]
```

Example:

```bash
versed query feemarket block-gas ...
```
...
```bash
versed query params subspace [subspace] [key] [flags]
```

Example:

```bash
versed query params subspace feemarket ElasticityMultiplier ...
```
