...
This document specifies the internal `x/claims` module of the Openverse Hub.

The `x/claims` module is part of the Openverse [Rektdrop](https://openverse.blog/the-openverse-rektdrop-abbe931ba823)
...
For the Openverse Rektdrop, users are required to claim their airdrop by participating in core network activities.
...
The Openverse [Rektdrop](https://openverse.blog/the-openverse-rektdrop-abbe931ba823) is the genesis airdrop
for the EVMOS token to Cosmos Hub, Osmosis and Ethereum users.

> The end goal of Openverse is to bring together the Cosmos and Ethereum community
...
After staking Openverse tokens (i.e delegating), the corresponding proportion will be airdropped to the user's balance
...
However, since Openverse only supports Ethereum keys and not default Tendermint keys,
this process differs for Ethereum and Cosmos eligible users.

#### Ethereum Users

Openverse shares the coin type (`60`) and key derivation (Ethereum `secp256k1`) with Ethereum.
...
need to perform a "cross-chain attestation" of their Openverse address.
...
The recipient Openverse address of this IBC transfer is the address, that the tokens will be airdropped to.

:::warning
**IMPORTANT**

Only submit an IBC transfer to an Openverse address that you own. Otherwise, you will lose your airdrop allocation.
:::
...
The user votes on a Governance proposal using their Openverse account.
...
The user deploys or interacts with a smart contract using their Openverse account or send a transfer using their Web3 wallet.
...
4. Check if package is from a sent NON EVM channel and sender and recipient
   ...
   well, which is not supported on Openverse. To prevent funds getting stuck,
...
7. Handle one of four cases by comparing sender and recipient addresses with each other
   ...
   To compare both addresses, the sender address's bech32 human readable prefix (HRP) is replaced with `openverse`.
...
The `EVMChannels` parameter describes the list of Openverse channels
...
Find below a list of `versed` commands added with the `x/claims` module.
...
```bash
versed query claims total-unclaimed [flags]
```
...
```bash
versed query claims records [flags]
```
...
```bash
versed query claims record ADDRESS [flags]
```
...
```bash
versed query claims params [flags]
```
...
|--------|--------------------------------------------|--------------------------------------------------|
| `gRPC` | `openverse.claims.v1.Query/TotalUnclaimed`     | Gets the total unclaimed tokens from the airdrop |
| `gRPC` | `openverse.claims.v1.Query/ClaimsRecords`      | Gets all registered claims records               |
| `gRPC` | `openverse.claims.v1.Query/ClaimsRecord`       | Get the claims record for a given user            |
| `gRPC` | `openverse.claims.v1.Query/Params`             | Gets claims params                               |
| `GET`  | `/openverse/claims/v1/total_unclaimed`         | Gets the total unclaimed tokens from the airdrop |
| `GET`  | `/openverse/claims/v1/claims_records`          | Gets all registered claims records               |
| `GET`  | `/openverse/claims/v1/claims_records/{address}` | Gets a claims record for a given user            |
| `GET`  | `/openverse/claims/v1/params`                  | Gets claims params                               |
