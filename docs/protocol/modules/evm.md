...
The `x/evm` module is part of the [ethermint library](https://pkg.go.dev/github.com/openverse/ethermint).
...
### Openverse as Geth implementation

Openverse contains an implementation of the [Ethereum protocol in Golang](https://geth.ethereum.org/docs/getting-started)
...
Openverse can be run as a node to compute state transitions with the EVM.
Openverse supports Geth's standard [Ethereum JSON-RPC APIs](https://docs.openverse.network/develop/api/ethereum-json-rpc/methods)
...
The JSON-RPC method [`eth_call`](https://docs.openverse.network/develop/api/ethereum-json-rpc/methods#eth-call) allows you
...
The Openverse implementation is similar and makes use of the gRPC query client which is included in the Cosmos SDK:

1. `eth_call` request is transformed to call the `func (e *PublicAPI) Call` function using the `eth` namespace
2. [`Call()`](https://github.com/openverse/ethermint/blob/main/rpc/namespaces/ethereum/eth/api.go#L639) calls `doCall()`
3. [`doCall()`](https://github.com/openverse/ethermint/blob/main/rpc/namespaces/ethereum/eth/api.go#L656)
   transforms the arguments into a `EthCallRequest` and calls `EthCall()` using the query client of the evm module.
4. [`EthCall()`](https://github.com/openverse/ethermint/blob/main/x/evm/keeper/grpc_query.go#L212)
   transforms the arguments into a `ethtypes.message` and calls `ApplyMessageWithConfig()
5. [`ApplyMessageWithConfig()`](https://github.com/openverse/ethermint/blob/d5598932a7f06158b7a5e3aa031bbc94eaaae32c/x/evm/keeper/state_transition.go#L341)
...
The implementation of this interface is what makes Openverse EVM compatible.
...
Openverse adapted the [go-ethereum journal implementation](https://github.com/ethereum/go-ethereum/blob/master/core/state/journal.go#L39)
...
Openverse uses the Cosmos `KVStore` (key-value store) and Cosmos SDK `Keeper` to facilitate state transitions.
...
Its main difference is that the one on Openverse uses a custom `Storage` type
...
Addresses use the **[EIP55](https://eips.ethereum.org/EIPS/eip-55)** hex **[format](https://docs.openverse.network/protocol/concepts/accounts#address-formats-for-clients)**
on `genesis.json`.
...
### Use Case: Call Native ERC20 Module on Openverse

Here is an example taken from the Openverse [erc20 module](erc20.md)
...
In the case of Openverse, the default denomination is the **atto openverse**.
In terms of precision, `EVMOS` and `ETH` share the same value,
*i.e.* `1 EVMOS = 10^18 atto openverse` and `1 ETH = 10^18 wei`.

:::tip
Note: SDK applications that want to import the EVM module as a dependency
will need to set their own `evm_denom` (i.e not `"aopenverse"`).
:::
...
Find below a list of `versed` commands added with the `x/evm` module.
...
```bash
versed query evm code ADDRESS [flags]
```

```bash
# Example
$ versed query evm code 0x7bf7b17da59880d9bcca24915679668db75f9397
...
```bash
versed query evm storage ADDRESS KEY [flags]
```

```bash
# Example
$ versed query evm storage 0x0f54f47bf9b8e317b214ccd6a7c3e38b893cd7f0 0 --height 0
...
```bash
versed tx evm raw TX_HEX [flags]
```

```bash
# Example
$ versed tx evm raw 0xf9ff74c86aefeb5f6019d77280bbb44fb695b4d45cfe97e6eed7acd62905f4a85034d5c68ed25a2e7a8eeb9baf1b84
...
For an overview on the JSON-RPC methods and namespaces supported on Openverse,
please refer to [https://docs.openverse.network/develop/api/ethereum-json-rpc/methodsl](https://docs.openverse.network/develop/api/ethereum-json-rpc/methods)
