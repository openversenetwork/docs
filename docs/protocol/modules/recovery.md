...
Recover tokens that are stuck on unsupported Openverse accounts.

## Abstract

This document specifies the  `x/recovery` module of the Openverse Hub.

The `x/recovery` module enables users on Openverse to recover locked funds
that were transferred to accounts whose keys are not supported on Openverse.
This happened in particular after the initial Openverse launch (`v1.1.2`),
where users transferred tokens to a `secp256k1` Openverse address via IBC
in order to [claim their airdrop](claims.md).
To be EVM compatible,
[keys on Openverse](https://docs.openverse.network/protocol/concepts/accounts#openverse-accounts) are generated
...
An example is the`eth_secp256k1`used by Ethereum and Openverse chain for generating public keys.
...
// Bech32 "openverse" address
ethsecpAddrOpenverse := sdk.AccAddress(ethPk.PubKey().Address()).String()

// We can also change the HRP to use "cosmos"
ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)
```

The above example code demonstrates a simple user account creation on Openverse.
...
please check the [accounts section](https://docs.openverse.network/protocol/concepts/accounts#openverse-accounts)
in the official Openverse documentation.

### Stuck funds

The primary use case of the `x/recovery` module is to enable the recovery of tokens,
that were sent to unsupported Openverse addresses.
These tokens are termed “stuck”, as the account’s owner cannot sign transactions
that transfer the tokens to other accounts.
The owner only holds the private key to sign transactions for its `eth_secp256k1` public keys on Openverse,
...
After the initial Openverse launch (`v1.1.2`), tokens got stuck from accounts with
and without claims records (airdrop allocation):

1. Osmosis/Cosmos Hub account without claims record sent IBC transfer to Openverse `secp256k1` receiver address
   ...
   **Recovery procedure**
    ...
    - The receiver can send an IBC transfer from their Osmosis / Cosmos Hub account (i.e `osmo1...` or `cosmos1...`)
      to its same Openverse account (`openverse1...`) to recover the tokens
      by forwarding them to the corresponding sending chain (Osmosis or Cosmos Hub)

2. Osmosis/Cosmos Hub account with claims record sent IBC transfer to Openverse `secp256k1` receiver address

   **Consequences**

    - IBC vouchers  from IBC transfer got stuck in the receiver’s balance
    - IBC Transfer Action was claimed
      and the EVMOS rewards were transferred to the receiver’s Openverse `secp256k1` account,
      resulting in stuck EVMOS tokens.
    - Claims record of the sender was migrated to the receiver’s Openverse `secp256k1` account
...
For Openverse the middleware stack ordering is defined as follows (from top to bottom):
...
1. User attempts to recover `1000aopenverse` that are stuck on the Openverse chain.
2. User sends `100uosmo` from Osmosis to Openverse through an IBC transaction.
3. Openverse receives the transaction, and goes through the IBC stack:
    1. **IBC transfer**: the `100uosmo` IBC vouchers are added to the user balance on openverse.
    2. **Claims Middleware**: since `sender=receiver` -> perform no-op
    3. **Recovery Middleware**: since `sender=receiver` -> recover user balance (`1000aopenverse` and `100uosmo`)
       by sending an IBC transfer from `receiver` to the `sender` on the Osmosis chain.
4. User receives `100uosmo` and `1000aopenverse` (IBC voucher) on Osmosis.
...
1. A user performs an IBC transfer to their own account by sending tokens from their address on an authorized chain
   (e.g. `cosmos1...`) to their openverse `secp2561` address (i.e. `openverse1`)  which holds the stuck tokens.
...
2. Check that the withdrawal conditions are met and skip to the next middleware if any condition is not satisfied:
    ...
    4. sender and receiver address belong to the same account as recovery
       is only possible for transfers to a sender's own account on Openverse.
...
4. Perform recovery to transfer the recipient’s balance back to the sender address with the IBC `OnRecvPacket` callback.
   There are two cases:
    ...
    1. First transfer from authorized source chain:
        ...
        2. sends over all Openverse native tokens
...
Find below a list of `versed` commands added with the `x/recovery` module.
...
```bash
versed query recovery params [flags]
```
...
| :----- | :------------------------------- | :-------------------- |
| `gRPC` | `openverse.recovery.v1.Query/Params` | `Get Recovery params` |
| `GET`  |   `/openverse/recovery/v1/params`    | `Get Recovery params` |
