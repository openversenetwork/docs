...
Make sure to look at [Openverse Governance](https://academy.openverse.network/community/governance/),
and specifically the [best practices](https://academy.openverse.network/community/governance/best-practices/).
:::

## Abstract

This document specifies the internal `x/erc20` module of the Openverse Hub.

The `x/erc20` module enables the Openverse Hub to support a trustless, on-chain bidirectional internal conversion of tokens
between Openverse' EVM and Cosmos runtimes, specifically the `x/evm` and `x/bank` modules.
This allows token holders on Openverse to instantaneously convert their native Cosmos `sdk.Coins`
...
With the `x/erc20` users on Openverse can
...
- transfer existing tokens on Ethereum and other EVM-based chains to Openverse
...
Holders of native Cosmos coins and IBC vouchers on the Openverse chain can convert their Coin into ERC20 Tokens,
which can then be used in Openverse EVM, by creating a `ConvertCoin` Tx.
Vice versa, the `ConvertERC20` Tx allows holders of ERC20 tokens on the Openverse chain
...
type TokenPair struct {
	...
	ContractOwner Owner `protobuf:"varint,4,opt,name=contract_owner,json=contractOwner,proto3,enum=openverse.erc20.v1.Owner" json:"contract_owner,omitempty"`
}
```
...
Note that the native Openverse coin cannot be registered,
as any coin including "evm" in its denomination cannot be registered.
Instead, the Openverse token can be converted by Nomand's wrapped Openverse (WEVMOS) contract.

1. User submits a `RegisterCoinProposal`
2. Validators of the Openverse Hub vote on the proposal using `MsgVote` and proposal passes
3. If Cosmos coin or IBC voucher exist on the bank module supply,
   create the [ERC20 token contract](https://github.com/openverse/openverse/blob/main/contracts/ERC20MinterBurnerDecimals.sol)
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg Openverse, Atom,
...
Find below a list of  `versed` commands added with the  `x/erc20` module.
You can obtain the full list by using the `versed -h` command.
A CLI command can look like this:

```bash
versed query erc20 params
```
...
```bash
versed tx gov submit-legacy-proposal register-coin METADATA_FILE [flags]
```
...
```bash
versed tx gov submit-legacy-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
versed tx gov submit-legacy-proposal toggle-token-conversion TOKEN [flags]
```
...
```bash
versed tx gov submit-legacy-proposal param-change PROPOSAL_FILE [flags]
```
...
| ------ | --------------------------------- | ------------------------------ |
| `gRPC` | `openverse.erc20.v1.Query/Params`     | Get erc20 params               |
| `gRPC` | `openverse.erc20.v1.Query/TokenPair`  | Get registered token pair      |
| `gRPC` | `openverse.erc20.v1.Query/TokenPairs` | Get all registered token pairs |
| `GET`  | `/openverse/erc20/v1/params`          | Get erc20 params               |
| `GET`  | `/openverse/erc20/v1/token_pair`      | Get registered token pair      |
| `GET`  | `/openverse/erc20/v1/token_pairs`     | Get all registered token pairs |

#### Transactions

| Verb   | Method                             | Description                    |
| ------ | ---------------------------------- | ------------------------------ |
| `gRPC` | `openverse.erc20.v1.Msg/ConvertCoin`   | Convert a Cosmos Coin to ERC20 |
| `gRPC` | `openverse.erc20.v1.Msg/ConvertERC20`  | Convert a ERC20 to Cosmos Coin |
| `GET`  | `/openverse/erc20/v1/tx/convert_coin`  | Convert a Cosmos Coin to ERC20 |
| `GET`  | `/openverse/erc20/v1/tx/convert_erc20` | Convert a ERC20 to Cosmos Coin |
