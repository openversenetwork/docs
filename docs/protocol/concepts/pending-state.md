...
Openverse is designed quite differently on this front as there is no concept of a "pending state".
Openverse uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Openverse cannot be ordered or cherry picked out from the Tendermint node
[mempool](https://docs.tendermint.com/v0.34/tendermint-core/mempool.html).

## Pending State Queries

Openverse will make queries which will account for any unconfirmed transactions present in a node's
