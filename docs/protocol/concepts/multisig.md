...
A **multisig account** is an Openverse account with a special key that can require more than one signature to sign transactions.
...
```bash
versed keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
versed keys add --multisig=p1,p2,p3 --multisig-threshold=2 multisig_address
versed keys add --multisig=p2,p3,p1 --multisig-threshold=2 multisig_address
```

Multisig addresses can also be generated on-the-fly and printed through the which command:

```bash
versed keys show --multisig-threshold=K name1 name2 name3 [...]
```
...
versed keys add \
    test3 \
    --pubkey=openversepub1addwnpepqgcxazmq6wgt2j4rdfumsfwla0zfk8e5sws3p3zg5dkm9007hmfysxas0u2
```
...
```sh
versed keys show multi

- name: multi
  type: multi
  address: openverse1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m
  pubkey: openversepub1ytql0csgqgfzd666axrjzq3mxw59ys6yqcd3ydjvhgs0uzs6kdk5fp4t73gmkl8t6y02yfq7tvfzd666axrjzq3sd69kp5usk492x6nehqjal67ynv0nfqapzrzy3gmdk27la0kjfqfzd666axrjzq6utqt639ka2j3xkncgk65dup06t297ccljmxhvhu3rmk92u3afjuyz9dg9
  mnemonic: ""
  threshold: 0
  pubkeys: []
```
...
versed tx bank send \
    test1 \
    openverse1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m \
    10000000000000000000aopenverse \
    --chain-id=openverse_9000-4 \
    --gas=auto \
    --fees=1000000aopenverse \
    --broadcast-mode=block
```

### Step 2: Create the multisig transaction

We want to send 5 EVMOS from our multisig account to `openverse1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft`.

```bash
versed tx bank send \
    openverse1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft \
    openverse157g6rn6t6k5rl0dl57zha2wx72t633axqyvvwq \
    5000000000000000000aopenverse \
    --gas=200000 \
    --fees=1000000aopenverse \
    --chain-id=openverse_9000-4 \
    ...
    "messages": [
      {
        ...
        "from_address": "openverse1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft",
        "to_address": "openverse157g6rn6t6k5rl0dl57zha2wx72t633axqyvvwq",
        "amount": [
          {
            "denom": "aopenverse",
...
versed tx sign \
    unsignedTx.json \
    --multisig=openverse1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m \
    --from=test1 \
...
```sh
versed tx broadcast signedTx.json \
    --chain-id=openverse_9000-4 \
    --broadcast-mode=block
```
