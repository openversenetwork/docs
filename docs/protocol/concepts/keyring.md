...
MY_VALIDATOR_ADDRESS=$(versed keys show dev0 -a)
```
...
:::warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Openverse due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | versed keys add me
yes $KEYPASSWD | versed keys show me
# start versed with keyring-backend flag
versed --keyring-backend=file start
```
