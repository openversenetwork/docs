...
Users need to pay a fee to submit transactions on the Openverse network.
As fees are handled differently on Ethereum and Cosmos,
it is important to understand how the Openverse blockchain implements an Ethereum-type fee calculation,
...
Also, note the fees that are paid for interacting with smart contracts on Openverse
...
### How are Gas and Fees Handled on Openverse?

Fundamentally, Openverse is a Cosmos SDK chain that enables EVM compatibility as part of a Cosmos SDK module. As a result of
...
Openverse logic. For the most part, fees are collected by the `fee_collector` module, then paid out to validators and
delegators. A few key distinctions are as follows:

1. Fee Market Module

    In order to support EIP-1559 gas and fee calculation on Openverse’ EVM layer, Openverse tracks the gas supplied for each
    ...
    Unlike on Ethereum, the `BaseFee` on Openverse is not burned, and instead is distributed to validators and delegators.
...
2. EVM Gas Refunds
    ...
    Openverse refunds a fraction (at least 50% by default) of the unused gas for EVM transactions to approximate the current
    behavior on Ethereum. [Why not always 100%?](https://github.com/openverse/ethermint/issues/1085)

3. Revenue Module

    Openverse developed the Revenue Module as a way to reward developers for creating useful dApps—any contract that is
    registered with Openverse’ Revenue Module rewards a fraction of the transaction fee (currently 95%) from each transaction
...
2. Nodes receive transactions for a subsequent block and gossip these transactions to peers
    * These can be sorted and prioritized by the included fee price (using EIP-1559 fee priority mechanics for EVM
    transactions - [code snippet](https://github.com/openverse/ethermint/blob/57ed355c985d9f3116aba6aabfa2ee0f3f38e966/app/ante/eth.go#L137)),
     to be included in the next block
3. Nodes run `BeginBlock` for the subsequent block
    * The FeeMarket module calculates the `BaseFee` ([code snippet](https://github.com/openverse/ethermint/blob/89fdd1984826ea524cb9b8feb089a99b6cfe8ace/x/feemarket/keeper/abci.go#L14))
...
Openverse is an EVM-compatible chain that supports Ethereum Web3 tooling. For this reason, gas consumption must be equatable
...
+++ https://github.com/openverse/ethermint/blob/098da6d0cc0e0c4cefbddf632df1057383973e4a/x/evm/keeper/state_transition.go#L188
...
For this same reason, in Openverse it is possible to send transactions with `0` fees for transaction
...
(i.e `ValidateBasic`) function as well as on the custom `AnteHandler` defined by Openverse.

### Gas Estimation

Ethereum provides a JSON-RPC endpoint `eth_estimateGas` to help users set up a correct gas limit in their transactions.

For that reason, a specific query API `EstimateGas` is implemented in Openverse. It will apply the transaction against the
...
+++ https://github.com/openverse/ethermint/blob/098da6d0cc0e0c4cefbddf632df1057383973e4a/x/evm/keeper/grpc_query.go#L100
...
Let’s say a user transfers tokens from Chain A to Openverse via IBC-transfer and wants to execute an Openverse transaction—however,
 they don’t have any Openverse tokens to cover fees. The Cosmos SDK introduced `Tips` as a solution to this issue; a user can
...
To cover transaction fees using a tip, this user can sign a transaction with a tip and no fees, then send the transaction
 to a fee relayer. The fee relayer will then cover the fee in the native currency (Openverse in this case), and receive the
...
## Dealing with gas and fees with the Openverse CLI

When broadcasting a transaction using the Openverse CLI client, users should keep into consideration the options available.
There are three flags to consider when sending a transaction to the network:

- `--fees`: fees to pay along with transaction; eg: 10aopenverse. Defaults to the required fees.
- `--gas`: the gas limit to set per-transaction; the default value is 200000.
- `--gas-prices`: gas prices to determine the transaction fee (e.g. 10aopenverse).
