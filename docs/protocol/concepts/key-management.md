...
## Mnemonics from the Openverse CLI

:::note
Before proceeding with the CLI, please insure you have `versed` installed. Installation instruction are located [here](./../../protocol/openverse-cli/single-node).
:::
...
```bash
versed keys add dev0
{
  "name": "dev0",
  "type": "local",
  "address": "openverse1n253dl2tgyhxjm592p580c38r4dn8023ctv28d",
...
```bash
$ versed keys add dev0-restored --recover
...
```bash
versed keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ versed keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ versed keys unsafe-import-eth-key dev0-imported ./dev0.export
