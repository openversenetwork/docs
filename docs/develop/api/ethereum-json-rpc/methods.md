...
Find below a list of JSON-RPC methods supported on Openverse, sorted by namespaces.
...
```json
 {"jsonrpc":"2.0","id":1,"result":"Openverse/0.1.3+/linux/go1.18"}
```
...
Make sure your `versed start` call is not using the flag `minimum-gas-prices` because this value will be used instead of the one set on the configuration file.
