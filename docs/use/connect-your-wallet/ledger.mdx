...
If you want to connect to Openverse mainnet and Openverse testnet, you can use the Ethereum Ledger app on Ledger Live by setting the chain ID.
...
| ------------- | --------------- |
| Openverse mainnet | `9001`          |
| Openverse testnet | `9000`          |
...
:::tip
Follow our [Metamask Guide](./metamask) to add the Openverse Mainnet and Testnet to your Settings
:::
...
To use your Ledger with the Openverse CLI, first connect your device to your computer, unlock it using your PIN, and open the Ethereum app.

Then, connect your Ledger to the CLI with `keys add` command, and select a name for your device:

```
versed keys add NAME --ledger
```

**Example:**

```
versed keys add myledger --ledger

- address: openverse1hnmrdr0jc2ve3ycxft0gcjjtrdkncpmmkeamf9
  name: myledger
...
```
versed tx bank send myledger openverse1hnmrdr0jc2ve3ycxft0gcjjtrdkncpmmkeamf9 100000aopenverse --fees 2000aopenverse --from myledger
```
...
### Step-by Cosmos transaction using Openverse.me

1. **Get your address in both encodings**

After connecting the Ledger wallet to Metamask and connecting to the [https://openverse.me](https://openverse.me) webpage, it will display our wallet formatted on `bech32` and `hex` representation, we need these values to make sure that the message that we are going to sign is the correct one.
...
- `msgs`: This is the content of the cosmos transaction, in this example, we need to make sure that we are using a MsgSend, and that the *to_address* is the one that we want to send the founds. Also, we can verify that we are actually sending *10000aopenverse* to that wallet.
...
This step should be done automatically by the same service that generated the message, in this case, [openverse.me](http://openverse.me) will broadcast the transaction for you.

![txsent.png](/img/txsent.png)

### Common errors

- Make sure that the Ethereum Ledger app is installed. The Cosmos Ledger app is not supported on the Openverse chain at the moment (see [FAQ](#faq)).
...
Cosmos `secp256k1` keys are not supported on Openverse with Ledger. Only Ethereum keys (`eth_secp256k1`) can be generated with Ledger.
