...
Connect your Metamask wallet with Openverse. 
...
:::note
If you are planning on developing on Openverse locally and you haven’t already set up your own local node, refer to [the quickstart tutorial](./../../protocol/openverse-cli/single-node), or follow the instructions in the [GitHub repository](https://github.com/openverse/openverse/).
:::
...
2. Search for `"Openverse"`
   ![chainlist.org website](/img/chainlist.png)
3. Click the `Connect Wallet` button under `Openverse Testnet`
...
```bash
versed keys unsafe-export-eth-key dev0
```
...
## Registering for Openverse Domains

Openverse Domains are modelled after the Ethereum Name Service (ENS), link [here](https://ens.domains/), Openverse users can register for an Openverse Domains with the help of [this](https://openverse.domains/) service. This service will use Metamask to connect and allow users to search for a name not taken. The registration process will take some EVMOS token.
